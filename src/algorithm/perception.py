...
class PerceptionModule:
    ...
    def start(self, debug_view=False, bus=None):
        """카메라·초음파 스레드를 띄운다. 하드웨어 import는 여기서 지연 로딩.
        선행=차선+YOLO(camera_loop), 후행=차선 전용(lane_camera_loop, AI HAT 불필요).
        debug_view=True 면 MJPEG 스트리밍 대시보드 활성화 (http://<IP>:8080/).
        bus 전달 시 /data 로 전 토픽이 노출된다."""
        from algorithm import sensing
        self._stop = threading.Event()
        role_str = "follower" if self.role == Role.FOLLOWER else "leader"
        ...
        self._threads = [
            threading.Thread(target=cam_loop,
                             ...
                             kwargs={"debug_view": debug_view, "bus": bus, "role": role_str},
                             daemon=True),
