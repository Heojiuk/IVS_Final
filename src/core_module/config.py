...
def load_key() -> bytes:
    """HMAC 사전공유키(32B)를 반환한다 — 프로젝트 루트 psk.key(32바이트 바이너리) 있으면 사용, 없으면 개발용 기본키.  파라미터 없음

    운영 전 psk.key 를 각 차량에 동일하게 배포(형상관리 제외 — .gitignore).
    """
    root = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))  # config.py → 프로젝트 루트
