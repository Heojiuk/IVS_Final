...
class DriveCommand:            # 판단이 내리는 주행 지시 (decision/command, IF-B2, 판단→주행·통신)
    stamp: float = 0.0
    behavior: DriveBehavior = DriveBehavior.CRUISE   # 무슨 동작을 할지 (위 DriveBehavior)
...
class EgoState:                                      # 자차의 현재 제어 상태 (motion/ego_state, IF-B4, 주행→통신)
    ...
    behavior: DriveBehavior = DriveBehavior.CRUISE   # 지금 수행 중인 동작 (판단 command 반영)
...
class V2VState:                          # WiFi로 받은 상대 차량 상태 (v2v/leader_state·follower_state, IF-B5, 통신→판단·주행)
    ...
    behavior: DriveBehavior = DriveBehavior.CRUISE   # 상대가 지금 하는 동작
